...
// apiDeliverMessage is one line of a fan-out request body (NDJSON: one message per line; a single
...
type peerQueue struct {
	mu    sync.Mutex
	url   string
	queue *util.LingerQueue[[]byte] // pre-marshaled apiDeliverMessage fragments
}

func (q *peerQueue) DeliverURL() string {
	q.mu.Lock()
	defer q.mu.Unlock()
	return q.url
}
